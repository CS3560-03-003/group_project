...
# --- Review Class --- 
class Review:
    def __init__(self, review_id: int, restaurant_id: int, customer_id: int, rating: float, review_content: str, review_date: str):
        self.review_id: int = review_id             # Primary key    
        self.restaurant_id: int = restaurant_id     # Foreign key to Restaurant
        self.customer_id: int = customer_id         # Foreign key to User
    ...
    def save(self):
        ...
        if not all([self.restaurant_id, self.customer_id, self.rating, self.review_content]):
        ...
        try:
            with get_DB_CONFIG_connection() as DB_CONFIG:
                ...
                """
                my_cursor.execute(query, (self.restaurant_id, self.customer_id, self.rating, self.review_content))
                DB_CONFIG.commit()
...
class Reply:
    ...
    def __init__(self, reply_id: int, review_id: int, owner_id: int, reply_content: str, reply_date: str):
        self.reply_id: int = reply_id               #Will hold ID once saved to DB
        self.review_id: int = review_id             # Foreign key to Review
        self.owner_id: int = owner_id                 # Foreign key to User  
    ...
    def save(self):
        ...
        try:
            with get_DB_CONFIG_connection() as DB_CONFIG:
                ...
                """
                my_cursor.execute(query, (self.review_id, self.owner_id, self.reply_content))
                DB_CONFIG.commit()
