...
-------------------------------
    ...
    def delete(review_id: int):
        """Deletes a review and its associated replies from the database."""
        try:
            with get_DB_CONFIG_connection() as DB_CONFIG:
                my_cursor = DB_CONFIG.cursor()

                my_cursor.execute("DELETE FROM replies WHERE reviewID = %s", (review_id,))

                my_cursor.execute("DELETE FROM reviews WHERE reviewID = %s", (review_id,))

                DB_CONFIG.commit()
                my_cursor.close()
                print(f"Review #{review_id} and its replies were deleted.")
                return True
...
class Restaurant:
    ...
    def delete(restaurant_id: int):
        """Deletes a restaurant, including all its reviews and replies."""
        try:
            with get_DB_CONFIG_connection() as DB_CONFIG:
                my_cursor = DB_CONFIG.cursor()
                ...
                delete_replies_query = """
                    DELETE replies 
                    FROM replies 
                    INNER JOIN reviews ON replies.reviewID = reviews.reviewID
                    WHERE reviews.restaurantID = %s
                """
                my_cursor.execute(delete_replies_query, (restaurant_id,))

                my_cursor.execute("DELETE FROM reviews WHERE restaurantID = %s", (restaurant_id,))

                my_cursor.execute("DELETE FROM restaurant WHERE restaurantID = %s", (restaurant_id,))
            ...
            print(f"Error deleting restaurant: {err}")
            return False
...
class Owner:
    ...
    def update_account(self, new_username, new_password, new_email, new_full_name):
        """Updates the owner's account details."""
        try:
            with get_DB_CONFIG_connection() as DB_CONFIG:
                my_cursor = DB_CONFIG.cursor()

                check_query = "SELECT ownerID FROM Owners WHERE (username = %s OR email = %s) AND ownerID != %s"
                my_cursor.execute(check_query, (new_username, new_email, self.owner_id))
                if my_cursor.fetchone():
                ...
                query = """
                    ...
                    WHERE ownerID = %s
                """
                my_cursor.execute(query, (new_username, new_password, new_email, new_full_name, self.owner_id))
                DB_CONFIG.commit()
            ...
            print(f"Error updating owner: {err}")
            return False

    def delete_account(self):
        """Deletes the owner AND all their restaurants."""
        try:
            with get_DB_CONFIG_connection() as DB_CONFIG:
                my_cursor = DB_CONFIG.cursor()
                my_cursor.execute("SELECT restaurantID FROM restaurant WHERE ownerID = %s", (self.owner_id,))
                results = my_cursor.fetchall()

                for row in results:
                    r_id = row[0]
                    Restaurant.delete(r_id)

                my_cursor.execute("DELETE FROM Owners WHERE ownerID = %s", (self.owner_id,))
                DB_CONFIG.commit()
                return True

        except mysql.connector.Error as err:
            print(f"Error deleting owner: {err}")
            return False
...
class Customer:
    ...
    def delete_account(self):
        """Deletes the customer account and all their reviews."""
        try:
            with get_DB_CONFIG_connection() as DB_CONFIG:
                my_cursor = DB_CONFIG.cursor()

                my_cursor.execute("SELECT reviewID FROM reviews WHERE customerID = %s", (self.customer_id,))
                user_reviews = my_cursor.fetchall()

                for (review_id,) in user_reviews:
                    my_cursor.execute("DELETE FROM replies WHERE reviewID = %s", (review_id,))

                my_cursor.execute("DELETE FROM reviews WHERE customerID = %s", (self.customer_id,))

                my_cursor.execute("DELETE FROM Customers WHERE customerID = %s", (self.customer_id,))

                DB_CONFIG.commit()
                print(f"Customer {self.username} and their history deleted.")
                return True
